...
| 6.12 | **Screen recording on a real device**, showing each feature | PARTIAL — [one minute on a real phone](https://github.com/mj789-98/skill-trial-blitz/releases/download/v1.0.3/screen-recording-v1.0.3.mp4) against the live backend: Blitz rounds in both games, a loss, a break-even, Pop Shot's moving basket. Screenshots from the same phone cover the [insufficient-balance lobby](screenshots/insufficient-balance.png) the [server-unreachable screen](screenshots/cannot-reach-server.png), and a [request failing mid-session in airplane mode](screenshots/request-failed-offline.png). Not yet shown: a Chicken Run cash-out win |
